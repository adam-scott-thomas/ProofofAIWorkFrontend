...
import { useUploads, usePool, useProjects, useTriggerClustering } from "../../hooks/useApi";
...
export default function UploadPool() {
  ...
  const { mutate: triggerClustering, isPending: clusteringPending } = useTriggerClustering();

  const uploads: any[] = Array.isArray(uploadsData) ? uploadsData : (uploadsData?.items ?? []);
  const totalConversations = pool?.total ?? 0;
  const unassigned = pool?.unassigned ?? 0;
  const allProjects: any[] = Array.isArray(projectsData) ? projectsData : (projectsData?.items ?? []);
  const suggestedCount = allProjects.filter((p: any) => p.status === "suggested").length;
      ...
      <div className="p-8">
        ...
        <div className="mb-6 grid grid-cols-3 gap-4">
          ...
          <Card className="border border-[rgba(0,0,0,0.08)] bg-white p-4 shadow-sm">
            ...
            <div className="mt-1 text-2xl tracking-tight">{suggestedCount}</div>
          </Card>
        </div>
        ...
        <Card className="mb-6 border border-[rgba(0,0,0,0.08)] bg-white p-6 shadow-sm">
          ...
          <div className="flex gap-3">
            <Button variant="outline" onClick={() => triggerClustering()} disabled={clusteringPending}>
              Free Clustering
              <span className="ml-2 text-[11px] text-[#717182]">Rule-based</span>
            </Button>
            <Button onClick={() => triggerClustering()} disabled={clusteringPending}>
              {clusteringPending ? "Clustering..." : "AI Clustering"}
              {!clusteringPending && <span className="ml-2 text-[11px]">$7</span>}
            </Button>
          </div>
        </Card>
        ...
        <Card className="border border-[rgba(0,0,0,0.08)] bg-white shadow-sm">
          ...
          {!uploadsLoading && uploads.length === 0 && (
            <div className="px-6 py-8 text-center text-[13px] text-[#717182]">
              No uploads yet. Upload your first conversation export to get started.
            </div>
          )}
                  ...
                  <div className="flex items-start justify-between gap-6">
                    <div className="flex-1">
                      <div className="mb-2 flex items-center gap-3">
                        <div className="text-[14px]">{upload.filename ?? upload.file_name}</div>
                      ...
                      <div className="flex items-center gap-4 text-[13px] text-[#717182]">
                        ...
                        {(upload.conversation_count ?? upload.conversationCount) > 0 && (
                          <span>{upload.conversation_count ?? upload.conversationCount} conversations</span>
                        )}
                        {(upload.uploaded_at ?? upload.uploadedAt) && (
                          <span>
                            {new Date(upload.uploaded_at ?? upload.uploadedAt).toLocaleDateString('en-US', {
                              month: 'short',
                              day: 'numeric',
                              year: 'numeric',
                              hour: '2-digit',
                      ...
                      {upload.status === "parsing" && upload.progress != null && (
                        <div className="mt-3">
